# Sprite baker demo build orchestrator
...
3. **WebGL build** — `Unity -batchmode -executeMethod SpriteBakerDemo.BuildTools.BuildCli.BuildWebGL`. Live phase progress from Unity's `DisplayProgressbar:` log markers. JSON build report at `Tools/Build/output/report-*.json`.
...
1. https://github.com/sinanata/unity-3d-to-sprite-baker → **Settings** → **Pages**.
2. Source: **Deploy from a branch**.
3. Branch: **`gh-pages`**, folder: **`/ (root)`**.
4. Save. URL: https://sinanata.github.io/unity-3d-to-sprite-baker/
...
- The atlas baker needs `mesh.isReadable == true` to read `SkinnedMeshRenderer` bone weights. The Kenney character imports without an embedded `AnimatorController`. Both invariants are enforced by `Assets/Demo/Editor/DemoModelImporter.cs` and `Assets/Demo/Editor/KenneyControllerBuilder.cs` — if either didn't run, force-reimport `Assets/Demo/Resources/Models/` or use **Sprite Baker Demo → Rebuild Kenney Controller**.
- The atlas shader at `Assets/SpriteBaker/Resources/Shaders/AtlasCutout.shader` is registered with `GraphicsSettings.AlwaysIncludedShaders` by `Assets/Editor/EnsureBuildShaders.cs` so WebGL stripping doesn't remove it.
